  ...
  for (;;)
    {
	 ...
	 //ROI part__________________(doesn't work on saved video)____________________
...
//velocity part__________________________________________________________________________ 
vector<Point>centerold(boundRect.size());
vector<int>vx(boundRect.size());
vector<int>vy(boundRect.size());
vector<int>v(boundRect.size());
vector<int>mindist(boundRect.size());

for(i=0;i<boundRect.size();i++)
{
if((getTickCount()/getTickFrequency())-t1>0.5)
{
for(j=0;j<boundRect.size();j++)
{
if(boundRect[i].height>70 || boundRect[i].width>70)//to only process big rects
{
 
 if((pow((((center[j].x-centerold[i].x)*(center[j].x-centerold[i].x))+((center[j].y-centerold[i].y)*(center[j].y-centerold[i].y))),0.5))<mindist[i])
 {
 mindist[i]=(pow((((center[j].x-centerold[i].x)*(center[j].x-centerold[i].x))+((center[j].y-centerold[i].y)*(center[j].y-centerold[i].y))),0.5));
 }
} 
 if(boundRect[i].height>70 || boundRect[i].width>70)//to only process big rects
{
 if((pow((((center[j].x-centerold[i].x)*(center[j].x-centerold[i].x))+((center[j].y-centerold[i].y)*(center[j].y-centerold[i].y))),0.5))<=2*mindist[i])
 {
 v[i]=mindist[i];
 centerold[i]=center[i];
 t1=getTickCount()/getTickFrequency();
 mindist[i]=1000;
 }
 }
 else
...
if(v[i]>=600)//numerical parameters vary
{
	putText(drawing,"Abnormal",Point(15,90),FONT_HERSHEY_SIMPLEX,1,Scalar(0,255,255),3);//put any drawing function of Mat before imshow
	color=Scalar(0,255,255);
}
//circle(drawing,center[i],10,(0,255,255),2,8);
if(v[i]>5)
{
	circle(drawing,center[i],8,Scalar(0,255,255),2,8);
	//circle(frame,center[i],8,Scalar(0,255,255),2,8);
	for(i=0;i<boundRect.size();i++)
	{
	    cout<<"Velocity of "<<i<<" is"<<v[i]<<"\n";
	    //circle(drawing,center[i],10,(255,255,0),2,8);
		//putText(frame,"High",center[i],FONT_HERSHEY_SIMPLEX,0.5,Scalar(0,255,255),1);
		putText(drawing,"High",center[i],FONT_HERSHEY_SIMPLEX,0.5,Scalar(0,255,255),1);
		line(drawing,Point(0,0),center[i],Scalar(255,255,0),0.5,8);
		//line(frame,Point(0,0),center[i],Scalar(255,255,0),0.5,8);
		}
